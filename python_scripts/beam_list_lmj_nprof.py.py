...
# initialize data
diag_dir = '/ccc/scratch/cont002/dam/martbert/Polaris/Data/lmj_list_beams_numprofs_4'
...
# read the results
ind_profiles = np.array([0, 1, 2, 3], dtype=int)
...
for ind_profile in ind_profiles :
    ...
    #index = np.argmin(rms)
    mini = rms[index]
    print("For target {:d} best config is {:d} with sigma rms = {:.2f}".format(ind_profile, index, mini))
    ...
    if with_pointing_markers:
        ...
        filename = diag_dir + "/" + diag_dir.split('/')[1] + "_prof{:d}_beams.txt".format(ind_profile)
        ...
        fig_name = "/" + diag_dir.split('/')[1] + '_prof{:d}_beams.png'.format(ind_profile)
        plt.savefig(diag_dir + fig_name, dpi=150, bbox_inches='tight')
